...
class SyncTask(private val mContext: Context, private val callback: SyncTaskListener, private val userId: Int) : AsyncTask<Void, Int, String>() {
    ...
    override fun doInBackground(vararg voids: Void): String {
        ...
        while (treeCursor.moveToNext()) {
            val localTreeId = treeCursor.getString(treeCursor.getColumnIndex("tree_id"))
            ...
            */
            ...
            if (postResult != null) {
                ...
                try {
                    this.mContext.sendBroadcast(Intent(Intent.ACTION_MEDIA_MOUNTED, Uri.parse("file://" + Environment.getExternalStorageDirectory())))
                } catch (e: Exception) {
                    e.printStackTrace()
                }
