...
class DataFragment : Fragment(), View.OnClickListener, SyncTask.SyncTaskListener {
    ...
    fun updateData() {
        ...
        treeCursor.moveToFirst()
        updateTrees!!.text = treeCursor.getString(treeCursor.getColumnIndex("updated"))
    ...
    internal inner class UpdateLocalDb : AsyncTask<List<UserTree>, Void, Void>() {
        ...
        override fun doInBackground(lists: Array<List<UserTree>>): Void? {
            ...
            for (tree in trees) {
                ...
                mDatabaseManager.delete("pending_updates", "tree_id = ?", arrayOf(tree.id))
                mDatabaseManager.closeDatabase()
            }
            return null
        }
        ...
        override fun onPostExecute(aVoid: Void) {
            ...
            for (entry in 0 until fm.backStackEntryCount) {
                Log.d("CheckFragmentBackStack", "Found fragment: " + fm.getBackStackEntryAt(entry).name)
            }
