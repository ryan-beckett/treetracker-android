...
class SettingsFragment : Fragment(), OnClickListener, OnCheckedChangeListener, TextWatcher {
    ...
    private var storageBrowseButton: Button? = null
    private var storagePathTextView: TextView? = null
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?,
        ...
        storageBrowseButton = v.findViewById(R.id.fragment_settings_storage_button) as Button
        storageBrowseButton!!.isEnabled = false
        storageBrowseButton!!.setOnClickListener {
            val intent = Intent(this@SettingsFragment.context, FolderPicker::class.java)
            startActivityForResult(intent, CHOOSE_FILE_REQUESTCODE)
        }

        storageSwitch = v.findViewById(R.id.fragment_settings_storage_switch) as Switch
        storageSwitch!!.setOnCheckedChangeListener { buttonView, isChecked ->
            if (isChecked) {
                //set storage settings in preferences to use external storage
                storageBrowseButton!!.isEnabled = true
            } else {
        ...
        storagePathTextView = v.findViewById(R.id.fragment_settings_storage_textview) as TextView
        ...
        for (i in 0 until manualRadioSettings.childCount) {
            ...
            if (rb != null) {
                ...
                if (Integer.parseInt(rb.text.toString()) == mSharedPreferences!!.getInt(ValueHelper.MIN_ACCURACY_GLOBAL_SETTING, ValueHelper.MIN_ACCURACY_DEFAULT_SETTING)) {
                    Log.i("rb text", rb.text.toString())
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == CHOOSE_FILE_REQUESTCODE && resultCode == Activity.RESULT_OK) {
            val path = data!!.extras!!.getString("data")
            //TODO Configure storage path here...
            storagePathTextView!!.text = path
            Toast.makeText(this.context, "External Storage is set.", Toast.LENGTH_SHORT)
        }
    }
    ...
    companion object {
        ...
        private val CHOOSE_FILE_REQUESTCODE = 1
    }
